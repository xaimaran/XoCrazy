...
namespace ThemeForge
{
    ...
    [InstalledProductRegistration("#110", "#112", "1.0.3")]
    [ProvideMenuResource("Menus.ctmenu", 1)]
    [ProvideToolWindow(typeof(ThemeForgeToolWindow), Style = VsDockStyle.Tabbed, Window = ToolWindowGuids.SolutionExplorer)]
    ...
    public sealed class ThemeForgePackage : AsyncPackage
    {
        private static readonly string DiagnosticLog =
            Path.Combine(Path.GetTempPath(), "themeforge-load.log");
        ...
        {
            Log("InitializeAsync entered; assembly=" + typeof(ThemeForgePackage).Assembly.Location);
            try
            {
        ...
        private ThemeForgeToolWindow OpenToolWindow()
        {
            ThreadHelper.ThrowIfNotOnUIThread();

            var window = FindToolWindow(typeof(ThemeForgeToolWindow), 0, create: true) as ThemeForgeToolWindow;
