...
namespace ThemeForge.Core
{
    ...
    internal static class ThemeStore
    {
        ...
        public static readonly string CurrentPath = Path.Combine(Directory, "current.themeforge.json");
        ...
        /// the new folder, so this answers once and then stops mattering.
        /// </summary>
        public static string LegacyPath(string current)
        {
            try
            {
                var legacy = Path.Combine(LegacyDirectory, Path.GetFileName(current));
